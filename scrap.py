...
def info_livre(url_du_livre, cat):
    ...
    if response.ok:
            with open('books/' + cat + '.csv','a', encoding='utf-8') as file: #j'ouvre le fichier books + cat qui changera a chaque fois + le type de fichier, en mode append
    ...
    if response.ok:
        ...
        for ultag in soup.find_all('ul', {'class': 'nav nav-list'}):
            ...
            for litag in ultagul.find_all('li'):
                ...
                with open('books/' + cat + '.csv', 'w', encoding="utf-8") as file:
